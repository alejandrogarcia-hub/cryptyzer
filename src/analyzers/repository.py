...
class GitHubAnalyzer:
    ...
    def _check_rate_limit(self) -> None:
        ...
        if remaining == 0:
            ...
            raise Exception(f"GitHub API rate limit exhausted. Resets in {wait_time/60:.1f} minutes")
    ...
    def _categorize_branch_type(self, branch_name: str) -> BranchType:
        ...
        if any(keyword in name_lower for keyword in ["refactor", "refactoring", "refact"]):
    ...
    def _categorize_pr_type(self, title: str, body: str, labels: List[str]) -> PullRequestType:
        ...
        if any(keyword in combined_text for keyword in ["feature", "feat", "enhancement"]):
            return PullRequestType.FEATURE
        if any(keyword in combined_text for keyword in ["fix", "bug", "issue #"]):
            return PullRequestType.BUGFIX
        if any(keyword in combined_text for keyword in ["hotfix", "critical", "urgent"]):
            return PullRequestType.HOTFIX
        if any(keyword in combined_text for keyword in ["test", "testing"]):
            return PullRequestType.TEST
        if any(keyword in combined_text for keyword in ["refactor", "refactoring", "refact"]):
    ...
    async def analyze_repository(self, repo_name: str) -> RepositoryMetrics:
        try:
            ...
            logger.info({"message": "Starting repository analysis", "repository": repo_name})
            ...
            type_counts = {pr_type: {
                "open": 0,
                "merged": 0
            } for pr_type in PullRequestType}
